...
- ✅ 사업주 콘솔의 체불 기록. 누군가 증명을 발행하는 순간 기록은 공개되고 영구히 남지만, 정작
  ...
  당사자에게 먼저 보이도록 했습니다.
- ✅ 임베드 가능한 신뢰도 배지([`/api/badge`](./web/api/badge.ts)). 사업주가 채용 공고나 회사
  소개 페이지, README에 붙여 넣는 SVG 이미지이며, 복사용 스니펫은 각 사업주 프로필에 있습니다.
  불러올 때마다 컨트랙트를 다시 읽으므로 좋았던 한 주의 캡처가 될 수 없습니다. "사업주가 채용
  공고에 점수를 표시할 수 있다"는 문서상의 약속을 실제로 가능하게 만든 부분입니다.
- ✅ 경로별 링크 미리보기([`/api/meta`](./web/api/meta.ts)). 클라이언트 렌더링 SPA는 모든 경로에
  같은 `index.html`을 내려주므로 공유된 사업주 프로필과 증빙 기록의 미리보기가 서로 구분되지
  않았고, 그 이전에는 `index.html`에 `og:` 태그 자체가 없어 미리보기가 아예 없었습니다. 이제 알려진
  크롤러는 user-agent로 판별해 체인 상태로 생성한 카드를 받습니다. 아직 텍스트 전용입니다 —
  경로별 미리보기 *이미지*에는 래스터 렌더러가 필요하며, 그 이유는
  [`api/meta.ts`](./web/api/meta.ts)에 적어 두었습니다.
...
- 노동청 · 노동조합 파일럿: 현직 노무사의 증빙 페이지 검토. ✅ 함께 예정했던 PDF 내보내기는
  검토보다 먼저 완료했습니다. 증빙 페이지는 이제 A4 페이지 문서(`@page`, 항목이 페이지 경계에서
  잘리지 않음, 직인과 미지급액 색상 유지)로 출력되며, 증빙 번호가 들어간 파일명으로 저장되고,
  화면의 하이퍼링크가 종이에서 잃어버리는 검증 주소와 전체 주소를 함께 인쇄합니다. PDF
  라이브러리가 아니라 브라우저 인쇄 엔진으로 렌더링하므로 한글이 이미지가 아닌 실제 벡터
  텍스트로 남아 담당자가 선택·검색할 수 있습니다 — [`web/src/lib/print.ts`](./web/src/lib/print.ts).
